...
server.tool(
  'send_media',
  'Send an image file to the user or group immediately. The file must exist on the container filesystem (e.g. in /workspace/group/).',
  {
  ...
  async (args) => {
    ...
    return { content: [{ type: 'text' as const, text: `Media sent: ${args.file_path}` }] };
  },
);
