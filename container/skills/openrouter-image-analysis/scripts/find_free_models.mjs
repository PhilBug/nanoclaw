...
function isFree(model) {
  ...
  const byPricing = ["prompt", "completion", "request", "image"].every((field) =>
    isZero(pricing[field])
  );
...
async function main() {
  ...
  if (!resp.ok) {
    ...
    throw new Error(`OpenRouter API returned ${resp.status}: ${body.slice(0, 300)}`);
  }
  ...
  for (const model of data) {
    ...
    if (inputModalities.has("image") && outputModalities.has("text") && isFree(model)) {
